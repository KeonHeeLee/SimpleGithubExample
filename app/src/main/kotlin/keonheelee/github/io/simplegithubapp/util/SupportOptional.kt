package keonheelee.github.io.simplegithubapp.util

// SupportOptional 클래스는 하나의 값을 포함할 수 있음
// Empty와 Some 클래스만 SupportOptional 클래스를 상속할 수 있도록
// sealed class로 선언
sealed class SupportOptional<out T: Any>(private val _value: T?){

    // 클래스가 널 아닌 값을 가지고 있는지 여부를 반환
    val isEmpty: Boolean
        get() = null == _value

    // 값을 반환하기 전에 checkNotNull() 함수를 사용하여 널 여부를 확인
    // 널 값을 반환하려 하는 경우 IllegalStateException이 발생
    val value: T
...
// 널 값이 아니라면 Some 클래스로, 널 값이라면 Empty 클래스로 감싸줌
